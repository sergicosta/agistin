...
class EB(syst_element):
    ...
    def __init__(self, ID, p_max):
        ...
        self.p_g_max = p_max
        self.var = ['model.'+self.x[0]+' = pyo.Var(model.t, within=pyo.NonNegativeReals, bounds=('+str(-self.p_g_max)+', '+str(self.p_g_max)+'),  initialize={k:0.0 for k in range(n)},)',]
...
class battery(syst_element):
    ...
    def __init__(self, system, ID, EB_loc, P_bat):
        super().__init__(ID, [f'P_bat{ID}'])
        self.system = system
        self.eb_loc = self.conn(EB_loc)
        self.P_bat = P_bat
        self.verification = True
        self.var = ['model.'+self.x[0]+' = pyo.Var(model.t, within=pyo.NonNegativeReals, bounds=(1e-6, '+str(self.P_bat)+'),   initialize={k:'+str(0.5*self.P_bat)+'   for k in range(n)},)',]
...
if __name__ == "__main__":
    ...
    sgr_sud.add_pump_simple(25e4, 2, 0.5, 0, 0)
