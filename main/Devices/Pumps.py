...
def RealPump(b, t, data, init_data):
    ...
    In RealPump it's added the limits of working of the pump.  
    .. math:
        H(t) = (n(t)/n_n)^2\cdot A - B \cdot Q(t)^2
    ...
    b.Ph   = pyo.Var(t, initialize=init_data['Pe'], bounds=(0, data['Pmax']), within=pyo.NonNegativeReals)
    b.Pe   = pyo.Var(t, initialize=init_data['Pe'], bounds=(0, data['Pmax']), within=pyo.NonNegativeReals)
    b.alpha= pyo.Var(t, initialize=1, within=pyo.NonNegativeReals)
    b.beta = pyo.Var(t, initialize=0, within=pyo.NonNegativeReals)
    ...
    def Constraint_bin_sum(_b,_t):
        return _b.alpha[_t] + _b.beta[_t] == 1
    b.c_bin1 = pyo.Constraint(t, rule = Constraint_bin_sum)
    
    def Constraint_bin_product(_b, _t):
