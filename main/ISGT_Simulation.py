...
dfw2 = pd.concat([dfw]*(365-298),ignore_index=True)
dfs = pd.concat([dfs]*183,ignore_index=True)

df = pd.concat([dfw1, dfs, dfw2], ignore_index=True)


meteo_file = 'G:/Unidades compartidas/CITCEA.PRJ_214_HorizonEurope_AGISTIN/06. documentacio tecnica/Dades/Datasets/Final/Meteocat/VA_ASCO.csv'
df_meteo = pd.read_csv(meteo_file)

df_meteo['date'] = pd.to_datetime(df_meteo['date'])
df_meteo = df_meteo[df_meteo['date'].dt.year == 2022]
df_meteo.set_index('date', inplace=True)
df_meteo = df_meteo['Irr'].resample('H').mean()



dec31_index = pd.date_range(start='2022-12-31 00:00:00', end='2022-12-31 23:00:00', freq='H')

df_meteo = df_meteo.reindex(df_meteo.index.union(dec31_index))

df_meteo.loc[dec31_index] = df_meteo.loc['2022-12-30'].values
...
def rellenar_nan(df):
    ...
    return df
df_meteo = rellenar_nan(df_meteo)
df_meteo.index = range(8760)




file_PVPC = './data/costs/export_PVPCTD_2024-09-30_15_51.csv'
df_PVPC = pd.read_csv(file_PVPC, sep=';')
df_PVPC['datetime'].astype(str)
df_PVPC['datetime'] = df_PVPC['datetime'].str[:-6]  #Delete 1+utc hour
df_PVPC['datetime'] = pd.to_datetime(df_PVPC['datetime'])
df_PVPC = df_PVPC.set_index('datetime')
df_PVPC = df_PVPC.drop(['id', 'name', 'geoid', 'geoname'], axis=1)
df_PVPC.index = range(8760)


file_exec = './data/costs/export_ExcedentariaDelAutoconsumo_2024-10-01_10_13.csv'
df_exec = pd.read_csv(file_exec,sep=';')
...
file = './results/ISGT/Base/ISGT.csv'
df = pd.read_csv(file)

df_param = pd.read_csv('./results/ISGT/Base/ISGT_param.csv')


h = 8760      #Test hours

SummerO = 91     #Initial summer day
SummerF = 244    #Final summer day

#Loading parameters

W0 = int(df_param['Reservoir1w.W0'].iloc[0])
zmax = int(df_param['Reservoir1w.zmax'].iloc[0])
zmin = int(df_param['Reservoir1w.zmin'].iloc[0])
pump1_eff = int(df_param['Pump1w.eff'].iloc[0]*100)
pump2_eff = int(df_param['Pump2w.eff'].iloc[0]*100)
Wmin = int(df_param['Reservoir1w.Wmin'].iloc[0])
Wmax = int(df_param['Reservoir1w.Wmax'].iloc[0])
...
k = [None]*h
df_res['k'] = k
df_res['Reservoir1.W'] = [0]*h
df_res['Reservoir1.z'] = [0]*h
df_res['Pipe1.H'] = [0]*h
df_res['Pump1.Pe'] = [0]*h
df_res['Pump2.Pe'] = [0]*h
...
df_res['Reservoir1.W'] = df_res['Reservoir1.W'].astype(float)
df_res['Reservoir1.z'] = df_res['Reservoir1.z'].astype(float)
...
for dia, df_dia in df_res.groupby(df_res['datetime'].dt.date):
    ...
    if Irr <= 1:
        k = 0
    elif Reservoir1_W - Irr + (Pump1 + Pump2) * 3600 < Wmin and (Pump1 > 1e-5 or Pump2 > 1e-5):
        k = (Wmin - Reservoir1_W + Irr) / ((Pump1 + Pump2) * 3600)
        if max_Pump1 * k > Qmax and max_Pump2 * k > Qmax:
            ...
            k = Qmax / max_Pump2
    elif Reservoir1_W - Irr + (Pump1 + Pump2) * 3600 > Wmax and (Pump1 > 1e-5 or Pump2 > 1e-5):
        k = (Wmax - Reservoir1_W + Irr) / ((Pump1 + Pump2) * 3600)
    ...
    Reservoir1_W += (Pump1 + Pump2) * k * 3600 - Irr

    df_res.loc[df_res['datetime'].dt.date == dia, 'Reservoir1.W'] = Reservoir1_W
...
for i in range(0, h):
    ...
    if i == 0:
        df_res.loc[t, 'Reservoir1.W'] = (df_param['Reservoir1w.W0'].iloc[0] + 
                                         (df_pumps.loc[0, 'Pump1.Qout'] + df_pumps.loc[0, 'Pump2.Qout']) * 3600)
    else:
        prev_t = df_res.index[i - 1]
        df_res.loc[t, 'Reservoir1.W'] = (df_res.loc[prev_t, 'Reservoir1.W'] +  
                                         (df_res.loc[t, 'Pump1.Qout'] + df_res.loc[t, 'Pump2.Qout']) * 3600 - 
                                         df_res.loc[t, 'Irrigation.Q'])
    
    # Variación del programa de bombeo
    df_res.loc[t, 'Reservoir1.z'] = ((df_res.loc[t, 'Reservoir1.W'] - df_param['Reservoir1w.Wmin'].iloc[0]) / 
                                     (df_param['Reservoir1w.Wmax'].iloc[0] - df_param['Reservoir1w.Wmin'].iloc[0]) * 
                                     (df_param['Reservoir1w.zmax'].iloc[0] - df_param['Reservoir1w.zmin'].iloc[0]) + 
                                     df_param['Reservoir1w.zmax'].iloc[0])

    df_res.loc[t, 'Pipe1.H'] = (df_res.loc[t, 'Reservoir1.z'] - df_param['ReservoirEbrew.zmax'].iloc[0] + 
...
daily_data['Reservoir1.W'] = df_res.groupby(df_res['datetime'].dt.date)['Reservoir1.W'].mean().values
...
# plt.figure(2)
# sns.lineplot(data=daily_data, x='date', y='Reservoir1.W')
# plt.xlabel('Día')
...
file = './results/ISGT/Case1/ISGT.csv'
df = pd.read_csv(file)

df_param = pd.read_csv('./results/ISGT/Case1/ISGT_param.csv')
...
for dia, df_dia in df_res.groupby(df_res['datetime'].dt.date):
    ...
    if Irr <= 1:
        k = 0
    elif Reservoir1_W - Irr + (Pump1 + Pump2-Turb1) * 3600 < Wmin and (Pump1 > 1e-5 or Pump2 > 1e-5):
        k = (Wmin - Reservoir1_W + Irr) / ((Pump1 + Pump2-Turb1) * 3600)
        if max_Pump1 * k > Qmax and max_Pump2 * k > Qmax:
            k = Qmax / max(max_Pump1, max_Pump2)
        elif max_Pump1 * k > Qmax:
            k = Qmax / max_Pump1
            ...
            k = Qmax / max_Pump2
    elif Reservoir1_W - Irr + (Pump1 + Pump2-Turb1) * 3600 > Wmax and (Pump1 > 1e-5 or Pump2 > 1e-5):
        k = (Wmax - Reservoir1_W + Irr) / ((Pump1 + Pump2-Turb1) * 3600)
        if min_Pump1 * k < Qmin and min_Pump2 * k < Qmin:
            k = 0
        elif min_Pump1 * k < Qmin:
            if min_Pump1 * k > Qmin * 0.5:
    ...
    Reservoir1_W += (Pump1 + Pump2-Turb1) * k * 3600 - Irr
...
for i in range(0,h):
    if i == 0:
        df_res.loc[0, 'Reservoir1.W'] = (df_param['Reservoir1w.W0'].iloc[0] + (df_res.loc[0,'Pump1.Qout'] + df_res.loc[0, 'Pump2.Qout'] - df_res.loc[0, 'Turb1.Qout'] ) * 3600)
        ...
        df_res.loc[i, 'Reservoir1.W'] = (df_res.loc[i-1,'Reservoir1.W'] +  (df_res.loc[i,'Pump1.Qout'] + df_res.loc[i, 'Pump2.Qout'] - df_res.loc[i, 'Turb1.Qout'] ) * 3600 - df_res.loc[i, 'Irrigation.Q'])
            
        if df_res.loc[i,'Reservoir1.W'] > Wmax:
            df_res.loc[i,'Pump2.Qout'] = 0
            df_res.loc[i,'Pump1.Qout'] = 0
        elif df_res.loc[i,'Reservoir1.W'] <=Wmin:
            df_res.loc[i,'Turb1.Qout'] = 0
        else:pass
        
        df_res.loc[i, 'Reservoir1.W'] = (df_res.loc[i-1,'Reservoir1.W'] +  (df_res.loc[i,'Pump1.Qout'] + df_res.loc[i, 'Pump2.Qout'] - df_res.loc[i, 'Turb1.Qout'] ) * 3600 - df_res.loc[i, 'Irrigation.Q'])

        df_res.loc[i, 'Reservoir1.z'] = ((df_res.loc[i, 'Reservoir1.W'] - df_param['Reservoir1w.Wmin'].iloc[0]) /(df_param['Reservoir1w.Wmax'].iloc[0] - df_param['Reservoir1w.Wmin'].iloc[0]) *(df_param['Reservoir1w.zmax'].iloc[0] - df_param['Reservoir1w.zmin'].iloc[0]) + 
            df_param['Reservoir1w.zmax'].iloc[0])

        df_res.loc[i, 'Pipe1.H'] = (df_res.loc[i, 'Reservoir1.z'] - df_param['ReservoirEbrew.zmax'].iloc[0] + df_param['Pipe1w.K'].iloc[0] * ((df_res.loc[i, 'Pump1.Qout'] +  df_res.loc[i, 'Pump2.Qout'] +df_res.loc[i, 'Turb1.Qout']))**2)
...
for i in range(0,h):
    if i == 0:
        df_res.loc[0, 'Reservoir1.W'] = (df_param['Reservoir1w.W0'].iloc[0] + (df_res.loc[0,'Pump1.Qout'] + df_res.loc[0, 'Pump2.Qout'] ) * 3600)
        
    else:
        
        df_res.loc[i, 'Reservoir1.W'] = (df_res.loc[i-1,'Reservoir1.W'] +  (df_res.loc[i,'Pump1.Qout'] + df_res.loc[i, 'Pump2.Qout'] ) * 3600 -df_con.loc[i, 'Qirr'])
        
        df_res.loc[i, 'Reservoir1.z'] = ((df_res.loc[i, 'Reservoir1.W'] - df_param['Reservoir1w.Wmin'].iloc[0]) /(df_param['Reservoir1w.Wmax'].iloc[0] - df_param['Reservoir1w.Wmin'].iloc[0]) *(df_param['Reservoir1w.zmax'].iloc[0] - df_param['Reservoir1w.zmin'].iloc[0]) + 
            df_param['Reservoir1w.zmax'].iloc[0])

        df_res.loc[i, 'Pipe1.H'] = df_res.loc[i, 'Reservoir1.z'] - df_param['ReservoirEbrew.zmax'].iloc[0] + df_param['Pipe1w.K'].iloc[0] * (((df_res.loc[i, 'Pump1.Qout'] +  df_res.loc[i, 'Pump2.Qout']))**2)
...
for i in range(0,h):
    if i == 0:
        ...
        df_res.loc[i, 'Reservoir1.W'] = (df_res.loc[i-1,'Reservoir1.W'] +  (df_res.loc[i,'Pump1.Qout'] + df_pumps.loc[i, 'Pump2.Qout'] - df_res.loc[i, 'Turb1.Qout'] ) * 3600 -df_res.loc[i, 'Irrigation.Q'])
        ...
        df_res.loc[i, 'Reservoir1.W'] = (df_res.loc[i-1,'Reservoir1.W'] +  (df_pumps.loc[i,'Pump1.Qout'] + df_pumps.loc[i, 'Pump2.Qout'] - df_pumps.loc[i, 'Turb1.Qout'])*df_res.loc[i,'k'] * 3600 -df_con.loc[i, 'Qirr'])
