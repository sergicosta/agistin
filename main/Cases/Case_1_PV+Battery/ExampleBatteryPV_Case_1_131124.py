...
# time
l_t = list(range(24)) #999 #TODO this should be inferred from the number of rows in the excel time series,
...
def obj_fun(m):
    return  ((sum(((m.Battery.P_FCRCharge[t]*22) + (m.Battery.P_FCRDisCharge[t])*-44 for t in l_t)) )  - (cost_new_battery_MW*m.Battery.Pdim_FCR) - (cost_new_battery_MWh*m.Battery.Edim_FCR)) - (cost_new_pv*m.PV.Pdim) #-(m.Battery.P_FCRDisCharge[t] + m.Battery.P_FCRDisCharge[t] + m.Grid.P[t])
