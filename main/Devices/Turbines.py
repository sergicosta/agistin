...
def Turbine(b, t, data, init_data):
    ...
    b.Ph   = pyo.Var(t, initialize=init_data['Pe'], bounds=(-data['Pmax'], 0), within=pyo.NonPositiveReals)
    b.Pe   = pyo.Var(t, initialize=init_data['Pe'], bounds=(-data['Pmax']*data['eff'], 0), within=pyo.NonPositiveReals)
...
def DiscreteTurbine(b, t, data, init_data):
    ...
    b.eff   = pyo.Param(initialize=data['eff'])
    b.Pmax  = pyo.Param(initialize=data['Pmax'])
    b.Qstep = pyo.Param(initialize=data['Pmax']/(9810*init_data['H'][0])/10)
    ...
    b.Ph   = pyo.Var(t, initialize=init_data['Pe'], bounds=(-data['Pmax'], 0), within=pyo.NonPositiveReals)
    b.Pe   = pyo.Var(t, initialize=init_data['Pe'], bounds=(-data['Pmax']*data['eff'], 0), within=pyo.NonPositiveReals)
    b.Nq   = pyo.Var(t, initialize=0, bounds=(0,10), within=pyo.NonNegativeIntegers)
