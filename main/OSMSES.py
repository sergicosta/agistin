...
# Objective function
def obj_fun(m):
	return sum((m.Grid.Pbuy[t]*m.cost[t] - m.Grid.Psell[t]*m.cost[t]/2) for t in l_t ) + m.PV.Pdim*cost_pv_P + m.Battery.Edim*cost_bat_E + m.Battery.Pdim*cost_bat_P
...
df.plot(y=['Pump1.Qout','Pump2.Qout','Irrigation1.Qout'], kind='bar', stacked=False, ylabel='Q', ax=ax2, 
        color=['lightgrey','lightgrey','k'], edgecolor='dimgray', label=['P1','P2','Irr'], sharex=True)
bars = ax2.patches
patterns =('////', '\\\\\\\\',None)
...
df.apply(lambda x: x/9810).plot(y=['Pump1.Pe','Pump2.Pe','PV.P', 'Grid.P'], kind='bar', stacked=True, ax=ax1, ylabel='P',
                                color=['lightgrey','lightgrey','dimgrey','k'], edgecolor='dimgray')
bars = ax1.patches
patterns =('////', '\\\\\\\\',None,None)
hatches = [p for p in patterns for i in range(len(df))]
for bar, hatch in zip(bars, hatches):
...
sns.barplot(data=df, x='t', y=df['Battery.P']/9810, label='Power', color='darkgrey')
...
ax = df_param[['PV.Pmax','Battery.Pmax','Battery.Emax']].apply(lambda x: x/9810).transpose().plot(kind='bar',stacked=False, 
...
plt.legend(['Max.', 'Inst.', 'Dim.'],loc='upper center', bbox_to_anchor=(0.5, -0.3),
          fancybox=False, shadow=False, ncol=5)
plt.ylabel('Sizing')
plt.xticks(range(3), labels=['PV $P$','Bat. $P$','Bat. $E$'], rotation=0)
