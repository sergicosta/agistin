...
data_filename = "ExampleBatteryPV168H"
...
# time
l_t = list(range(168)) #999 #TODO this should be inferred from the number of rows in the excel time series,
...
cost_new_battery_MW  = 900
cost_new_battery_MWh = 400
cost_new_pv = 10


def obj_fun(m):
    return ((sum((abs(m.Battery.P_FCRCharge[t]) + abs(m.Battery.P_FCRDisCharge[t]))*0.25*abs(m.Battery.FCR_reBAP[t]) + m.Battery.Pdim_FCR*m.Battery.FCR_Remuneration[t] for t in l_t))
