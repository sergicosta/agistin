...
# data_p['eff']=0.859
# RealPump(m.Pump3w, m.tw, data_p, init_p) #pump de reserva
...
data_pv = {'Pinst':236e3, 'Pmax':236e3, 'forecast':df_meteo_jan['Irr'].head(T)/1000, 'eff':0.98} # PV
...
for df in [df_S]:
    ...
    df.apply(lambda x: x/1000).plot(y=['PV.P','Pump2.Pe'], kind='bar', stacked=True, ax=ax1, ylabel='P (kW)',
                                color=[cbcolors[2],cbcolors[3]], edgecolor=None)
    df.apply(lambda x: x/1000).plot(y=['Grid.P','Rev1.Pe'], kind='bar', stacked=True, ylabel= 'P(kW)', ax=ax1, color= [cbcolors[7],cbcolors[0]])
    ...
    for bar, hatch in zip(bars, hatches):
        bar.set_hatch(hatch)
    ax1.legend(['_','$\hat{P}_{PV}$','$P_{PV}$','$P_{pump2}$','$P_{Grid}$','$P_{rev1}$'],loc='upper center', bbox_to_anchor=(0.5, -0.2),
