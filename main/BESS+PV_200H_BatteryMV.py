...
# Import builder
from BuilderBatteryPV_main import data_parser, builder
...
data_filename = "BESS+V_200H_BatteryMV"
...
# time
l_t = list(range(200)) #TODO this should be inferred from the number of rows in the excel time series,
...
#""
cost_new_battery_MW  = 1000 #900
cost_new_battery_MWh = 1000 #400
cost_new_pv = 80
cost_FCR_reBAP_pos = 10
cost_FCR_reBAP_neg = 0
cost_FCR_Remuneration = 0

#Objective function
def obj_fun(m):
	return ((sum((abs(m.Battery.P_FCRCharge[t])*cost_FCR_reBAP_neg + abs(m.Battery.P_FCRDisCharge[t])*cost_FCR_reBAP_pos) + m.Battery.Pdim_FCR *cost_FCR_Remuneration for t in l_t)))
